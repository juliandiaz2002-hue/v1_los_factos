...
class CategorySuggestionService:
    ...
    def _by_similarity(self, *, detalle_norm: str, monto_abs_clp: int) -> CategorySuggestion | None:
        ...
        best: tuple[float, Movimiento] | None = None
        for item in candidates:
            text_score = SequenceMatcher(None, detalle_norm, item.detalle_norm).ratio()
        ...
        category = self.session.get(Categoria, movement.categoria_id)
        if not category:
            return None

        return CategorySuggestion(
            categoria=category,
            source="similarity_name_amount",
            confidence=round(float(score), 3),
            reason="Similitud por nombre + monto",
        )
